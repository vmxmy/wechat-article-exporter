...
- [ ] 17.9 Implement and validate the artifact-bound clean-room evidence gate: execute the extracted candidate binary for installation, clean-root creation, real legacy-Web migration, database baselines, controlled live QR login and restart persistence, synchronization, article/resource download, every export format with real local Chromium PDF, Cobra exit/JSON contracts, native-PTY TUI, stdio MCP, platform secret persistence, independent backup/restore, process-tree retired-domain exclusion, receipt leakage scanning, and OS-enforced offline workflows; produce one passing native platform receipt for each supported tuple and one passing aggregate five-target release receipt set. Fixture, source-harness, cross-compile, emulation, skipped, or relabeled-live evidence does not complete this task.
