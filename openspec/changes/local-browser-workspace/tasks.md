...
- [ ] 3.7 Add cross-adapter contract tests comparing browser API outcomes with Cobra/TUI/MCP application seams. The browser currently delegates through the shared `application.Workspace` facade and focused handler tests cover its bounded DTO and persistent-job response seams; direct outcome-comparison contract tests across Cobra, TUI, and MCP have not yet been added, so this broad item remains open.
