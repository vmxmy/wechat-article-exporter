export const zhCN = {
  ...
  settings: {
    ...
    credentials: { title: '凭据', description: '仅列出元数据。输入的凭据字段只写入，导入后会从浏览器中清除。', empty: '没有可用的凭据元数据。', import: '导入凭据', remove: '移除', removeConfirmation: (id: string) => `remove-credential:${id}`, removeConfirmationLabel: '移除此凭据的精确确认字符串', removeConfirmationHint: '请原样输入此值。它只会移除所选凭据元数据及其密钥字节。', confirmRemove: '移除凭据', cancelRemove: '取消移除', nickname: '昵称', cookie: 'Cookie（仅写入）', optional: '可选', imported: '已导入凭据。密钥值未保留在浏览器中。', file: '凭据 JSON 文件', fileHint: '所选文件仅在本机流式导入，随后会从表单中清除。不会保留文件名或内容。', fileImported: '凭据文件已导入。浏览器未保留密钥值或文件详情。', columns: { account: '账号', kind: '类型', status: '状态', updated: '更新时间' } },
