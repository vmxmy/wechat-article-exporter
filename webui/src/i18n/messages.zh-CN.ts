export const zhCN = {
  ...
  resources: {
    accounts: {
      ...
      actions: { title: '账号操作', description: '发现账号、保存本地账号记录，或为一个已选账号启动同步。', search: '搜索发现', fakeid: '账号 fakeid', name: '账号名称', alias: '别名', discover: '发现账号', discoveryResults: '发现结果', discoveryEmpty: '没有匹配此发现搜索的账号。', useCandidate: '使用此候选账号', candidateSelected: (name: string) => `已选择 ${name}。请检查账号详情，然后保存本地账号。`, add: '保存账号', saved: (name: string) => `已保存 ${name}。现在可以开始同步。`, edit: '更新所选账号', remove: '删除所选账号', sync: '同步所选账号', downloadManifest: '下载账号清单', importManifest: '导入账号清单', manifestHint: '所选 JSON 清单仅在本机上传，随后立即从控件中清除。本页面不会保留文件名或内容。', manifestImported: (added: number, merged: number, unchanged: number) => `账号清单已导入：新增 ${added} 个，合并 ${merged} 个，未变更 ${unchanged} 个。`, manifestFailed: '无法导入账号清单。', selectOne: '请先仅选择一个账号。', deleteTitle: '删除所选账号', deleteConfirm: '删除所选本地账号记录？此操作无法撤销。', deleteConfirmation: (ids: readonly string[]) => `delete-accounts:${ids.join(',')}`, deleteConfirmationLabel: '删除这些账号的精确确认', deleteConfirmationHint: '请准确输入该值。它只会删除所选的本地账号记录。', confirmDelete: '删除账号', cancelDelete: '保留账号', actionFailed: '无法完成账号操作。' }
    },
    albums: {
      title: '专辑',
