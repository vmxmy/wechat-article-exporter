...
import { DenseRegion, MobileResourceRow, PageHeader, ResponsiveDataTable, SectionStack } from '../../components/presentation'
import type { MessageCatalog } from '../../i18n'
import { createSelectionColumn, type ResourceColumnRole } from '../../lib/presentation'
...
export function ResourceTable<T extends { readonly id?: string; readonly name?: string }>({ eyebrow, messages, columns, query, pageIndex, onPageChange, onSelectionChange, preserveSelectionAcrossPages = false, maximumSelectedIDs, selectionScope, hideHeader = false, headerActions, tableToolbar, emptyState, selectionLabel }: ResourceTableProps<T>) {
  ...
  const selectionColumn = useMemo(
    () => createSelectionColumn<T>({
      ...
      selectRowLabel: (row) => messages.selectRow(resourcePrimaryText(row.original, row, columns))
    }),
    [columns, messages]
  )
  useEffect(() => {
  ...
  return (
    <SectionStack as="section" gap="section" aria-labelledby={hideHeader ? undefined : titleID}>
      {hideHeader ? null : <PageHeader eyebrow={eyebrow} title={messages.title} titleId={titleID} description={messages.description} actions={headerActions} />}
      <DenseRegion>
...
function resourceMetadata<T>(row: Row<T>, columns: readonly ColumnDef<T>[]) {
  return columns.flatMap((column) => {
    const role = resourceColumnRole(column)
    if (role === 'selection' || role === 'primaryText' || role === 'secondaryText' || role === 'description' || role === 'status' || role === 'actions') return []
