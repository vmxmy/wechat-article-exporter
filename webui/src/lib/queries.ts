...
import {
  ...
  authorizeDefaultExportDirectory, beginLogin, completeLogin, controlJob, createExportDirectory, deleteAccounts, ingestURL, logout, openExportOutput, pollLogin, saveAccount, searchAccounts, startExport, syncAccount, syncAccounts, updateAccount, verifyExport,
...
export function useWorkspaceMutations() {
  ...
  return {
    ...
    syncAccounts: useMutation({ mutationFn: ({ ids, mode }: { ids: readonly string[]; mode: AccountSyncMode }) => syncAccounts(ids, mode), onSuccess: refresh }),
    ...
    traverseAlbums: useMutation({ mutationFn: ({ albumIds, order, download }: { albumIds: readonly string[]; order: AlbumTraversalOrder; download: boolean }) => traverseAlbums(albumIds, order, download), onSuccess: refresh }),
