...
async function fulfillAPI(route: Route, url: URL, state: State) {
  ...
  if (url.pathname === '/api/v1/articles') {
    const articles = url.searchParams.get('offset') === '25'
      ? [{ id: 'article-fixture-3', title: 'Sanitized article three', accountId: 'account-fixture', accountName: 'Fixture Account', author: 'Fixture Author', publishedAt: now, state: 'ready' }]
  ...
  if (url.pathname === '/api/v1/jobs') return page(route, [{ id: 'job-fixture-1', kind: 'export', state: 'running', profile: 'fixture-profile', createdAt: now, updatedAt: now, counts: { completed: 1, total: 2 }, permittedActions: ['pause', 'cancel'] }])
