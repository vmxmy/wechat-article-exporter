...
## Current state on `vmxmy/wechat-article-exporter`

`ready-for-agent` and `wontfix` already exist. `needs-triage`, `needs-info`, and
`ready-for-human` do not — create them on first use:

    gh label create <name> --repo vmxmy/wechat-article-exporter
