...
账号页支持下载账户 manifest，也支持选择一个 JSON manifest 后本地上传、暂存并导入；页面随即清空文件选择，且不保留文件名或内容。凭据字段同样是仅写入的：除表单录入外，Settings 可上传一个受大小限制的 Credential JSON，导入后清空文件选择，列表只显示安全元数据。维护页面已接入备份创建、验证和通过一次性不透明 handle 下载 ZIP，另外还有完整性检查、GC 的计划/一次性确认执行、诊断读取，以及以不透明一次性 handle 下载的脱敏诊断包。
