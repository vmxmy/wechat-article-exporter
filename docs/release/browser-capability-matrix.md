...
| Account search, add/manage, sync, manifest transfer | Supported | Supported | Supported where policy permits | Supported | Browser account views use bounded API pages and durable jobs; it streams manifest download and accepts one bounded JSON upload into an opaque staged handle before import. |
...
| Album traversal and batch download/export | Supported | Supported | Supported where policy permits | Traversal, batch download, and selected-album export handoff supported; broader batch-export workflow not complete | Browser can list albums, traverse one selected album and queue its batch download; its selected-album action passes an opaque album selection to the export page. Use Cobra/TUI/MCP for broader batch-export workflows. |
