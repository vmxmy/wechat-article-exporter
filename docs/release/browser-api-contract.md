...
| Persistent job creation | `POST /accounts/{id}/sync`; `/ingest/url`; `/articles/download`; `/articles/metadata`; `/articles/comments`; `/articles/resources`; `/albums/{id}/traverse`; `/exports/start` | `202 Accepted` | The first seven routes return the shared persistent job DTO with its stable `id`; export start returns `{ "jobId": "<stable persistent job id>" }`. The handler only queues shared application work and never waits for job execution. |
