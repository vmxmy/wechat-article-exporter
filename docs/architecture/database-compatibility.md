...
| Schema 3 (`003_reply_checkpoints.sql`) | Current schema | Must open without schema mutation after migrations have been recorded. |
...
- discovery columns and the `reply_checkpoints` table/index exist;
...
1. Add the next contiguous `NNN_description.sql` file; do not modify an already released migration. Schema 3 moves
   `reply_checkpoints` out of runtime lazy DDL so databases already recorded at schema 2 receive the table on upgrade.
